...
This version has been slightly modified by Esha Datta for use in spinal cord gray matter segmentation.
"""
...
def evolve_visual(msnake, data_min, data_max, levelset=None, num_iters=20, background=None):
    """
    ...
    if background is None:
        ax1.imshow(msnake.data, cmap=ppl.cm.gray, vmin=data_min, vmax=data_max)
    else:
        ax1.imshow(background, cmap=ppl.cm.gray, vmin=data_min, vmax=data_max)
    ax1.contour(msnake.levelset, [0.5], colors='r')
    plt.axis('off')

    #Commented out following 2 lines
